...
func init() {
	...
	dbResetCmd.Flags().BoolVarP(&resetSeed, "seed", "s", false, "Runs a seed after the reset")
	dbResetCmd.Flags().BoolVarP(&resetTestData, "test-data", "t", false, "Adds test data (implies --seed)")
	dbResetCmd.Flags().BoolVarP(&resetForce, "force", "f", false, "Skips the confirmation prompt (for CI/CD)")
}
