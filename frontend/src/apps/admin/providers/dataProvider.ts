...
export const createDataProvider = (apiBaseUrl: string): DataProvider => {
  ...
  const baseProvider = jsonServerProvider(`${apiBaseUrl}`, httpClient);
  ...
  return {
    ...
    upload: async (resource: string, params: CreateParams) => {
      const url = `${apiBaseUrl}/${resolveResource(resource)}`;
    ...
    refund: async (resource: string, params: UpdateParams) => {
      ...
      const url = `${apiBaseUrl}/${resolveResource(resource)}/${params.id}/refund`;
      const options: HttpClientOptions = {
        method: "POST",
