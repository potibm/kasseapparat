...
import { Modal, ModalHeader, ModalBody, ModalFooter, Spinner } from "flowbite-react";
...
const PollingModal = ({ show, purchase, onClose, onConfirmed }) => {
    ...
    const cancelPayment = () => {
      ...
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        wsRef.current.send(JSON.stringify({ type: "cancel_payment", reader_id:  sumUpReaderId}));
    ...
    const current = statusInfo[status] || { color: "text-gray-600", icon: null, spinnerColor: "info", };

    useEffect(() => {

      const ws = new WebSocket(`ws://localhost:3001/api/v2/purchases/${purchase.id}/ws`);
      wsRef.current = ws;
    ...
    <Modal show={show} size="md" popup dismissible={false}>
      <ModalBody>
        <div className="text-center">
          ...
          <div  className={`text-center space-y-4 ${flash ? "animate__animated animate__headShake" : ""}`}>
            {status === "pending" && (
              <Spinner color={current.spinnerColor} size="xl" />
            )}
            <div className={`text-lg font-medium flex items-center justify-center gap-2 ${current.color}`}>
              {current.icon}
              <span>
      ...
      <ModalFooter>
          <MyButton color="failure" disabled={processing} onClick={() => cancelPayment()}>
            Abort Purchase
